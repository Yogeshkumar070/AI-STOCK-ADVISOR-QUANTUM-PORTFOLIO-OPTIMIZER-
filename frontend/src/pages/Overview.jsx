...
function CompanyLogo({ website, name, size = 64 }) {
  ...
  return (
    <div className="rounded-2xl flex items-center justify-center font-black shadow-sm border border-slate-200 flex-shrink-0"
...
/* === INDICES - Fetched from backend (no CORS issues) === */
function useIndices() {
  const [data, setData] = useState([
    { label: "NIFTY 50", price: null, chg: null, pct: null },
    { label: "BANK NIFTY", price: null, chg: null, pct: null },
    { label: "FIN NIFTY", price: null, chg: null, pct: null },
    { label: "SENSEX", price: null, chg: null, pct: null },
    { label: "MIDCPNIFTY", price: null, chg: null, pct: null },
  ]);
  useEffect(() => {
    let alive = true;
    const go = async () => {
      try {
        const res = await fetch("http://127.0.0.1:8000/stock/indices");
...
function IndexCards({ indices }) {
  return (
    <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 pt-6 pb-6">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {indices.map((idx, i) => {
          const has = idx.price !== null;
          const up = has && idx.chg >= 0;
          return (
            <div key={idx.label || i} className="bg-white border border-slate-200 rounded-xl px-5 py-4 shadow-[0_2px_10px_rgb(0,0,0,0.03)] flex flex-col justify-between">
      ...
      <div className="max-w-[1400px] mx-auto overflow-x-auto no-scrollbar">
        <div className="flex items-stretch min-w-max divide-x divide-slate-100">
          {indices.map((idx, i) => {
            ...
            const up = has && idx.chg >= 0;
            return (
              <div key={idx.label || i} className="flex items-center gap-3 px-6 py-2.5 hover:bg-slate-50/60 transition-colors cursor-default">
                <div>
    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden mb-6">
      <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-slate-50/50">
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none">{stockData.name || symbol}</h1>
        ...
        <div className="flex flex-col items-start lg:items-end gap-4">
          ...
          <div className="flex items-center gap-1.5 rounded-xl px-2 py-1.5 border border-slate-200 bg-white shadow-sm focus-within:ring-2 focus-within:ring-blue-100 transition-all">
            ...
            <input type="text" value={inputVal} onChange={(e) => setInputVal(e.target.value.toUpperCase())} onKeyDown={(e) => e.key === "Enter" && onNewSearch(inputVal)}
              placeholder="New ticker..." className="w-32 py-1 text-sm bg-transparent outline-none text-slate-800 font-semibold"/>
            <button onClick={() => onNewSearch(inputVal)} disabled={loading} className="bg-[#0066FF] text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-colors disabled:opacity-50 hover:bg-blue-700">Go</button>
          </div>
        </div>
      </div>
      <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <p className="text-[14px] text-slate-600 leading-relaxed font-medium">{displayDesc}</p>
          {truncated && (
          ...
          <div className="space-y-4">
            {details.map((item, i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-slate-500">{item.icon}<span className="text-xs font-semibold">{item.label}</span></div>
  ...
  return (
    <div className="relative overflow-hidden bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group border border-slate-200">
      ...
      <div className="px-5 py-5 flex items-center gap-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${iconBg} group-hover:scale-110 transition-transform duration-300`}>{icon}</div>
...
export default function Overview() {
  ...
  return (
    <div className="min-h-screen pb-20 bg-white">
      {!stockData && !loading && (
        <div className="animate-in fade-in duration-500 bg-gradient-to-b from-[#F4F7FB] via-white to-[#F8FBFF] w-full min-h-[calc(100vh-116px)]">
          <IndexCards indices={indices}/>
          <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6">
            <div className="min-h-[calc(100vh-280px)] flex flex-col items-center justify-center text-center px-6 py-16">
              <h1 className="font-bold text-[46px] md:text-[54px] text-slate-800 tracking-tight leading-tight mb-4 max-w-5xl">
              ...
              <div className="w-full max-w-[700px] flex items-center bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-200 px-4 py-3 focus-within:ring-4 focus-within:ring-blue-100 transition-all">
                ...
                <input type="text" value={symbol} onChange={(e) => setSymbol(e.target.value.toUpperCase())} onKeyDown={(e) => e.key === "Enter" && fetchStock()}
                  placeholder="Search for companies and stocks to analyse..." className="flex-1 py-3 text-[17px] outline-none text-slate-800 placeholder-slate-400"/>
                <button onClick={() => fetchStock()} className="hidden md:block bg-[#0066FF] text-white px-8 py-3.5 rounded-lg font-bold text-sm tracking-wide transition-all hover:bg-blue-700 hover:shadow-md">ANALYZE</button>
              </div>
              <div className="flex flex-wrap justify-center gap-3 mt-8 max-w-3xl">
                {quickLinks.map(ticker => (
                  <button key={ticker} onClick={() => { setSymbol(ticker); fetchStock(ticker); }}
